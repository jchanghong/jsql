...
object HelpStatement {
    ...
    fun handle(x: MySqlHelpStatement, c: OConnection) {


        val helpstatment = x.toString()
        if (helpstatment.contains("use")) {
            ...
            MSelect1Response.response(c, "help use", listOf("use database ,比如use db;"))
            return
        }
        if (helpstatment.contains("showdb")) {
            MSelect1Response.response(c, "help showdb", listOf("SHOW DATABASES [LIKE wild]\n" +
        ...
        if (helpstatment.contains("select")) {
            MSelect1Response.response(c, "help select", listOf("SELECT [STRAIGHT_JOIN] [SQL_SMALL_RESULT] [SQL_BIG_RESULT] [HIGH_PRIORITY]\n" +
        ...
        if (helpstatment.contains("create")) {
            MSelect1Response.response(c, "help create", listOf("CREATE DATABASE db_name CREATE DATABASE用给定的名字创建一个数据库。\n" + "CREATE [TEMPORARY] TABLE [IF NOT EXISTS] tbl_name [(create_definition,...)]\n" +
        ...
        if (helpstatment.contains("insert")) {
            MSelect1Response.response(c, "help insert", listOf("INSERT [LOW_PRIORITY | DELAYED] [IGNORE]\n" +
        ...
        if (helpstatment.contains("alter")) {
            MSelect1Response.response(c, "help alter", listOf("ALTER [IGNORE] TABLE tbl_name alter_spec [, alter_spec ...]\n" +
                    "\n" +
                    "alter_specification:\n" +
