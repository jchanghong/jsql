...
//        c.writeOkMessage("hello");
        ...
        if (helpstatment.contains("select")) {
            MSelect1Response.response(c, "help select", Collections.singletonList("SELECT [STRAIGHT_JOIN] [SQL_SMALL_RESULT] [SQL_BIG_RESULT] [HIGH_PRIORITY]\n" +
        ...
        if (helpstatment.contains("create")) {
            MSelect1Response.response(c, "help create", Collections.singletonList("CREATE DATABASE db_name CREATE DATABASE用给定的名字创建一个数据库。\n" + "CREATE [TEMPORARY] TABLE [IF NOT EXISTS] tbl_name [(create_definition,...)]\n" +
        ...
        if (helpstatment.contains("insert")) {
            MSelect1Response.response(c, "help insert", Collections.singletonList("INSERT [LOW_PRIORITY | DELAYED] [IGNORE]\n" +
        ...
        if (helpstatment.contains("alter")) {
            MSelect1Response.response(c, "help alter", Collections.singletonList("ALTER [IGNORE] TABLE tbl_name alter_spec [, alter_spec ...]\n" +
                    "\n" +
                    "alter_specification:\n" +
