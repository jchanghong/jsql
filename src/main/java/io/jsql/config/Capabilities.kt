...
interface Capabilities {
    companion object {
        ...
        val CLIENT_IGNORE_SIGPIPE = 4096

        // Client knows about transactions
        val CLIENT_TRANSACTIONS = 8192

        // Old flag for 4.1 protocol
        val CLIENT_RESERVED = 16384

        // New 4.1 authentication
        val CLIENT_SECURE_CONNECTION = 32768

        // Enable/disable multi-stmt support
        // 通知服务器客户端可以发送多条语句（由分号分隔）。如果该标志为没有被设置，多条语句执行。
        val CLIENT_MULTI_STATEMENTS = 65536

        // Enable/disable multi-results
        // 通知服务器客户端可以处理由多语句或者存储过程执行生成的多结果集。
        // 当打开CLIENT_MULTI_STATEMENTS时，这个标志自动的被打开。
        val CLIENT_MULTI_RESULTS = 131072

        val CLIENT_PLUGIN_AUTH = 0x00080000 // 524288
    }
